...
import com.cyber.punk.entity.summer_decoration.OakPlankVerticalEntity;
...
public class OakPlankVertical extends AbstractCustomBlock {
    public OakPlankVertical() {
        super(Properties.of(
                        Material.WOOD)
                .strength(2f,4.0f)
                .noOcclusion());
    ...
    public TileEntity createTileEntity(BlockState state, IBlockReader world) {
        return new OakPlankVerticalEntity();
    }
    ...
    protected boolean canPlaceBlockAt(World world, BlockPos pos, Direction facing) {
        ...
        switch (facing) {
            case NORTH:
            default:
                positions = new BlockPos[]{pos.relative(Direction.UP), pos.relative(Direction.WEST), pos.relative(Direction.EAST)};
                break;
            case SOUTH:
                positions = new BlockPos[]{pos.relative(Direction.UP), pos.relative(Direction.EAST), pos.relative(Direction.WEST)};
                break;
            case WEST:
                positions = new BlockPos[]{pos.relative(Direction.UP), pos.relative(Direction.NORTH), pos.relative(Direction.SOUTH)};
                break;
            case EAST:
                positions = new BlockPos[]{pos.relative(Direction.UP), pos.relative(Direction.SOUTH), pos.relative(Direction.NORTH)};
                break;
        }
