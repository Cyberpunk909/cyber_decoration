...
public class SciFiHorizontalWire extends AbstractCustomBlock {
    public SciFiHorizontalWire() {
        super(Properties.of(
                        Material.METAL)
                .strength(2f,4.0f)
                .noOcclusion());
    ...
    private static final VoxelShape SHAPE_N = Stream.of(
            Block.box(0, 0, -16, 14, 2, 14),
            Block.box(-16, 0, 4, 0, 2, 14)
    ...
    public TileEntity createTileEntity(BlockState state, IBlockReader world) {
        return new SciFiHorizontalWireEntity();
    }
        ...
        switch (facing) {
            ...
            default:
                positions = new BlockPos[]{
                        pos.relative(Direction.NORTH), pos.relative(Direction.WEST)};
                break;
            case SOUTH:
                positions = new BlockPos[]{
                        pos.relative(Direction.SOUTH), pos.relative(Direction.EAST)};
                break;
            case WEST:
                positions = new BlockPos[]{
                        pos.relative(Direction.WEST), pos.relative(Direction.SOUTH)};
                break;
            case EAST:
                positions = new BlockPos[]{
                        pos.relative(Direction.EAST), pos.relative(Direction.NORTH)};
                break;
        }
